...
from pycompss.api.parameter import COLLECTION_IN, Depth, Type, Cache
...
@task(blocks={Type: COLLECTION_IN, Depth: 2, Cache: True}, returns=np.array, cache_returns=True)
def _partial_sum_gpu(blocks_gpu, centers_gpu):
    import cupy as cp
 
    partials = np.zeros((centers_gpu.shape[0], 2), dtype=object)
    arr_gpu = Array._merge_blocks(blocks_gpu).astype(cp.float32)
    ...
    close_centers_gpu = None

    for center_idx, _ in enumerate(centers_gpu):
        indices = np.argwhere(close_centers == center_idx).flatten()
        partials[center_idx][0] = cp.asnumpy(cp.sum(arr_gpu[indices], axis=0))
...
def distance_gpu(a_gpu, b_gpu):
    ...
    dist_gpu = cp.empty(dist_shape, dtype=cp.float32)
