...
from EddlDistributedConmutativo import EddlDistributedConmutativo
...
class EncapsulatedFunctionsTensor(object):
    ...
    def build(self, net, optimizer, loss, metric, num_gpu=0, num_nodes=0):
        ...
        self.compss_object.build(net, optimizer, loss, metric, num_gpu, gpu=np.arange(num_gpu))
            ...
            for row in range(x_train.shape[0]):
                for col in range(x_train.shape[1]):
                    parameters_for_workers[j] = \
                        self.compss_object.train_cnn_batch_GPU(
                            parameters_for_workers[j],
    ...
    def fit_synchronous_every_n_epochs_with_GPU(self, x_train, y_train,
        ...
        gpu = np.zeros(self.num_gpu, dtype=np.int8)
        for i in range(num_epochs):
            j = 0
            if shuffle_blocks:
                rows = np.random.permutation(x_train.shape[0])
                cols = np.random.permutation(x_train.shape[1])
            gpu[auxiliar_gpus:auxiliar_gpus + self.num_gpus_per_worker] = 1
            for row in rows:
                for col in cols:
                    parameters_for_workers[j] = \
                        self.compss_object.train_cnn_batch_GPU(
                            ...
                            x_train.dtype,
                            num_batches_per_worker,
                            gpu_to_use=gpu, epoch=i,
                            shuffle_block_data=shuffle_block_data)
                    auxiliar_gpus = auxiliar_gpus + self.num_gpus_per_worker
                    if auxiliar_gpus == len(self.num_gpus):
                        auxiliar_gpus = 0
                    gpu[:] = 0
                    gpu[auxiliar_gpus:auxiliar_gpus +
        ...
        if shuffle_blocks:
            ...
            for i in range(num_epochs):
                ...
                for row in rows:
                    for col in cols:
                        parameters_for_workers[j] = \
                            self.compss_object.train_batch(
                                parameters_for_workers[j],
                                x_train.tensors[int(row)][int(col)],
                                y_train.tensors[int(row)][int(col)],
                                x_train.dtype,
