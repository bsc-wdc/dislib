...
def test_split(sample, y_s, feature, n_classes):
    ...
    l_frequencies = np.zeros((n_classes, size),
    ...
    r_weight = np.sum(np.square(np.cumsum(r_frequencies, axis=-1)), axis=0)[
               ::-1]

    l_length = np.arange(1, size + 1, dtype=np.int32)
    r_length = np.arange(size - 1, -1, -1, dtype=np.int32)  # type: np.ndarray
    r_length[
        size - 1] = 1  # Avoid div by zero, the right score will be 0 anyways
