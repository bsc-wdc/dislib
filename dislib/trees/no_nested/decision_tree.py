    ...
    def fit(self, x, y):
        ...
        for i in range(self.distr_depth):
            ...
            for idx, branch_data in enumerate(branches):
                ...
                node_info, results = _compute_split(x, y,
...
def generate_nodes_with_data_compressed_regression(node_info, found_solution,
    ...
    if (np.sum(y_l_occ) + np.sum(y_r_occ)) <= 4:
        ...
        node_info.set(_compute_leaf_info((np.sum(y_l) +
                                          np.sum(y_r)) / (np.sum(y_l_occ) +
                                                          np.sum(y_r_occ)),
                                         None,
                                          occurrences=np.sum(y_l_occ) +
                                                      np.sum(y_r_occ)))
...
def get_split_point_various_attributes_bucket(unique_values, total_length,
                                              number_split_points=100,
                                              split_computation="raw"):
    sample_blocks_list = []
    for idx, bucket in enumerate(unique_values):
        ...
        if total_length is not None:
            number_split_points_actual = int(
                number_split_points * len(bucket) / total_length)
        else:
