...
class BaseRandomForest(BaseEstimator):
    ...
    def fit(self, x, y):
        ...
        if isinstance(DecisionTreeRegressor, self.base_tree):
                ...
                for idx, branch_data in enumerate(branches[n_estimator]):
                    ...
                    if self.n_classes is not None:
                        ...
                        for k, sides in enumerate(results):
                            ...
                            splits_computed = []
                        self.nodes_info[n_estimator].append(node_info)
                    else:
        ...
        for n_estimator in range(self.n_estimators):
            for branch in branches[n_estimator]:
                ...
                construct_subtree(x, y, actual_node, self.try_features,
                                  ...
                                  random_state=self.random_state)
                self.nodes_info[n_estimator].append(actual_node)
    ...
    def save_model(self, filepath, overwrite=True, save_format="json"):
        ...
        >>> loaded_model = loaded_model.load_model('/tmp/model')
            ...
            for x_row in x._iterator(axis=0):
                ...
                for n_estimator in range(self.n_estimators):
                    tree_predictions.append(_predict_tree_class(
                        x_row._blocks, self.nodes_info[n_estimator],
                        0, self.n_classes))
