...
source_python = 'source /fefs/scratch/bsc19/bsc19776/opt/virtual_python-3.6.8/bin/activate'
...
def main():
    cmd = ("LC_ALL=en_US.UTF-8 enqueue_compss"
           ...
           " --scheduler=" + scheduler +
           " --pythonpath=" + scripts_dir + ":" + tests_dir + ":" + "/fefs/scratch/bsc19/bsc19776/opt/dislib" +
           ...
           " --sc_cfg=cte-arm.cfg"
           " --cpu_affinity=\"12-59\""
           " --extrae_config_file=/fefs/scratch/bsc19/bsc19776/experiments/test/extrae_basic.xml"
    ...
    final_cmd = ["pjsub", "-L", "node=1", "--step", "--sparam", "jnam=performance", "-x", "time_str=" + out,
                 os.path.join(scripts_dir, "postprocess.sh")]
    # print(' '.join(final_cmd))
    subprocess.run('cd ' + logdir + '; ' + ' '.join(final_cmd), shell=True, executable='/bin/bash')


def run_job(cmd, logdir):
    cd_log = 'cd ' + logdir
    proc = subprocess.run(cd_log + '; ' + source_python + '; ' + ' '.join(cmd) + '; deactivate', shell=True, executable='/bin/bash')
