...
def _validate_tensor(x):
    ...
    return (tl == x.tensor_shape and br0 == br[0] and x.n_tensors /
...
class TensorTest(unittest.TestCase):
    ...
    def test_iterate_rows(self):
        ...
        x_np = ds.random_tensors("np", (3, 3, 3, 11, 11))
        x_torch = Tensor(tensors=x_np.tensors, tensor_shape=x_np.tensor_shape, shape=x_np.shape, dtype=x_np.dtype)

        for i, h_block in enumerate(x_np._iterator(axis='rows')):
            computed = h_block
            expected = x_torch[i: (i + 1)]
            self.assertTrue(_validate_tensor(computed))
            self.assertTrue(_equal_valued_tensors(computed.collect(),
                                                  expected.collect()))

        x_torch = ds.random_tensors("torch", (3, 3, 3, 11, 11))
        x_np = Tensor(tensors=x_torch.tensors, tensor_shape=x_torch.tensor_shape, shape=x_torch.shape, dtype=x_torch.dtype)
    ...
    def test_iterate_cols(self):
        ...
        for i, v_block in enumerate(x_np._iterator(axis='columns')):
            expected = x_torch[:, i: (i + 1)]
            self.assertTrue(_validate_tensor_col(v_block))
            self.assertTrue(_equal_valued_tensors(v_block.collect(),
                                                  expected.collect()))

        x_np = ds.random_tensors("torch", (3, 3, 3, 11, 11))
        x_torch = Tensor(tensors=x_np.tensors, tensor_shape=x_np.tensor_shape, shape=x_np.shape, dtype=x_np.dtype)
    ...
    def test_indexing(self):
        ...
        aux = x[1, 2]
        self.assertEqual(aux.shape, (1, 1))
        self.assertEqual(aux.tensor_shape, (3, 11, 11))
        aux = x[0:0]
        self.assertEqual(aux.shape, (1, 1))
    ...
    def test_invalid_assignation(self):
        ...
        with self.assertRaises(ValueError):
            tensor_to_assign = np.array([[2, 3, 4, 5], [2, 3, 4, 5], [2, 3, 4, 5], [2, 3, 4, 5]])
        ...
        x_train_tensor = ds.random_tensors("np", (2, 2, 4, 4))
        with self.assertRaises(ValueError):
            x_train_tensor[0, 0] = tf.constant(np.array([[2, 3, 4, 5], [2, 3, 4, 5], [2, 3, 4, 5], [2, 3, 4, 5]]))
    ...
    def test_assignation(self):
        ...
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][0]).shape == (4, 4, 4))
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][0]).all() == np.ones((4, 4, 4)).all())
        ...
        self.assertTrue(x_train_tensor.tensor_shape == (4, 4, 4))
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][0]).shape == (4, 4, 4))
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[1][0]).all() == np.ones((4, 4, 4)).all())
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][0]).all() == np.ones((4, 4, 4)).all())
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][1]).all() == np.ones((4, 4, 4)).all())
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[1][1]).all() == np.ones((4, 4, 4)).all())

        x_train_tensor = ds.random_tensors("np", (2, 2, 4, 4, 4))
        x_train_tensor[0, 0, 0, 0, 1] = 8
        self.assertTrue(x_train_tensor.tensor_shape == (4, 4, 4))
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][0]).shape == (4, 4, 4))
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][0])[0, 0, 1] == 8)
        ...
        self.assertTrue(x_train_tensor.tensor_shape == (4, 4, 4))
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][0]).shape == (4, 4, 4))
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][0]).all() == np.ones((4, 4)).all())
        ...
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][0]).shape == (4, 4, 4))
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][0]).all() == np.ones((4, 4)).all())
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[1][0]).all() == np.ones((4, 4)).all())
        ...
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][0]).shape == (4, 4, 4))
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][0]).all() == np.ones((4, 4)).all())
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][1]).all() == np.ones((4, 4)).all())
        ...
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[0][1]).all() == np.ones((4, 4)).all())
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[1][0]).all() == np.ones((4, 4)).all())
        self.assertTrue(compss_wait_on(x_train_tensor.tensors[1][1]).all() == np.ones((4, 4)).all())

    def test_basic_operations(self):
        x_np = ds.from_array(np.array([[[1, 2, 3], [3, 2, 1]]]))
        x_np_pow = x_np**2
        x_np_pow = np.array(x_np_pow.collect())
        self.assertEqual(x_np_pow.all(), np.array([[np.array([1, 4, 9]), np.array([9, 4, 1])]]).all())
        with self.assertRaises(NotImplementedError):
            x_np**(2.21, 2)
        x_np_mul = x_np * 2
        x_np_mul = np.array(x_np_mul.collect())
        self.assertEqual(x_np_mul.all(), np.array([[np.array([1, 4, 6]), np.array([6, 4, 1])]]).all())
        with self.assertRaises(NotImplementedError):
            x_np*(2.21, 2)
        x_np_2 = ds.from_array(np.array([[[2, 4, 8], [8, 16, 32]]]))
        x_np_div = x_np_2 / 2
        x_np_div = np.array(x_np_div.collect())
        self.assertEqual(x_np_div.all(), np.array([[np.array([1, 2, 4]), np.array([4, 8, 16])]]).all())
        with self.assertRaises(NotImplementedError):
            x_np/(2.21, 2)
        x_add = x_np + x_np_2
        x_add = np.array(x_add.collect())
        self.assertEqual(x_add.all(), np.array([[np.array([3, 6, 11]), np.array([11, 18, 33])]]).all())
        x_sub = x_np - x_np_2
        x_sub = np.array(x_sub.collect())
        self.assertEqual(x_sub.all(), np.array([[np.array([-1, -2, -5]), np.array([-5, -14, -31])]]).all())
        with self.assertRaises(ValueError):
            x_np_2 = ds.from_array(np.array([[[2, 4, 8], [8, 16, 32], [1, 2, 3]]]))
            x_np + x_np_2
        with self.assertRaises(ValueError):
            x_np_2 = ds.from_array(np.array([[[2, 4, 8], [8, 16, 32, 4]]]))
            x_np + x_np_2
        with self.assertRaises(ValueError):
            x_np_2 = ds.from_array(np.array([[[2, 4, 8], [8, 16, 32], [1, 2, 3]]]))
            x_np - x_np_2
        ...
        x_np_2 = Tensor(tensors=[["String1", "String2"]], tensor_shape=(2, 2), dtype=np.float64)
    ...
    def test_rechunk(self):#TODO AMPLIAR COVERAGE
        np_array = np.random.rand(2, 2, 9, 3, 3)
        x_np = ds.from_array(np_array)
        tensor = ds.data.tensor.rechunk(x_np, 6, dimension=0)
        self.assertEqual(tensor.tensor_shape, (6, 3, 3))

        np_array = np.random.rand(2, 2, 38, 3, 3)
        x_np = ds.from_array(np_array)
        tensor = ds.data.tensor.rechunk(x_np, 8, dimension=0)
    ...
    def test_create_empty_tensor(self):
        ...
        self.assertEqual((len(empty_tensor.tensors), len(empty_tensor.tensors[0])), (2, 2))
    ...
    def test_load_dataset(self):
        ...
        self.assertTrue(isinstance(x_train_tensor.tensors[0][0], (np.ndarray, np.generic)))
        path = "tests/datasets/pt"
        x_train_tensor = ds.data.tensor.load_dataset(2, path)
        self.assertEqual(x_train_tensor.shape, (2, 2))
        x_train_tensor.collect()
        self.assertTrue(torch.is_tensor(x_train_tensor.tensors[0][0]))
        with self.assertRaises(NotImplementedError):
    ...
    def test_shuffle(self):
        ...
        self.assertTrue(x_train_tensor_2.shape == (2, 2) == (len(x_train_tensor_2.tensors), len(x_train_tensor_2.tensors[0])))
        x_train_tensor_2 = np.array(x_train_tensor_2.collect())
        self.assertFalse((x_train_tensor_2 == np.array(x_train_tensor.collect())).all())
        ...
        x_train_tensor_2, y_train_tensor_2 = ds.data.tensor.shuffle(x_train_tensor, y_train_tensor)
        x_train_tensor_2 = np.array(x_train_tensor_2.collect())
        y_train_tensor_2 = np.array(y_train_tensor_2.collect())
        self.assertFalse((x_train_tensor_2 == np.array(x_train_tensor.collect())).all())
        self.assertFalse((y_train_tensor_2 == np.array(y_train_tensor.collect())).all())

        x_train_tensor = ds.random_tensors("torch", (2, 2, 2, 2, 2, 2))
        x_train_tensor_2 = ds.data.tensor.shuffle(x_train_tensor)
        self.assertTrue(
            x_train_tensor_2.shape == (2, 2) == (len(x_train_tensor_2.tensors), len(x_train_tensor_2.tensors[0])))
        x_train_tensor_2 = np.array(x_train_tensor_2.collect())
        self.assertFalse((x_train_tensor_2 == np.array(x_train_tensor.collect())))
        ...
        y_train_tensor_2 = np.array(y_train_tensor_2.collect())
        self.assertFalse((x_train_tensor_2 == np.array(x_train_tensor.collect())))
        self.assertFalse((y_train_tensor_2 == np.array(y_train_tensor.collect())))
    ...
    def test_apply_to_tensors(self):
        ...
        self.assertTrue(x_train_tensor_2[0][0].all() == compss_wait_on(x_train_tensor.tensors[0][0]).all())
        self.assertTrue(x_train_tensor_2[0][1].all() == compss_wait_on(x_train_tensor.tensors[0][1]).all())
        self.assertTrue(x_train_tensor_2[1][0].all() == compss_wait_on(x_train_tensor.tensors[1][0]).all())
        self.assertTrue(x_train_tensor_2[1][1].all() == compss_wait_on(x_train_tensor.tensors[1][1]).all())
