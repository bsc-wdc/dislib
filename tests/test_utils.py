...
class UtilsTest(unittest.TestCase):
    ...
    def test_train_test_split_x(self):
        ...
        x_ds = ds.array(x, (8, 8))
        train, test = ds.data.train_test_split(x_ds)
    ...
    def test_train_test_split_xy(self):
        ...
        x_ds = ds.array(x, (8, 8))
        y_ds = ds.array(y, (8, 1))
        train, test, y_train, y_test = ds.data.train_test_split(x_ds, y_ds)
        self.assertTrue(train.shape[0] == int(40 * 0.75))
        self.assertTrue(test.shape[0] == int(40 * 0.25))
        self.assertTrue(train.shape[1] == int(40 * 0.75))
        self.assertTrue(test.shape[1] == int(40 * 0.25))
    ...
    def test_train_test_split_excep(self):
        ...
        with self.assertRaises(ValueError):
            ds.data.train_test_split(x, y, test_size=1.25, train_size=1)
        with self.assertRaises(ValueError):
            ds.data.train_test_split(x, y, test_size=0.95, train_size=0.1)
        with self.assertRaises(ValueError):
            ds.data.train_test_split(x, y=np.zeros(2, 2), test_size=0.95,
                                     train_size=0.1)
        with self.assertRaises(ValueError):
            ds.data.train_test_split(x=np.zeros(2, 2), test_size=0.95,
                                     train_size=0.1)
