...
class GridSearchCVTest(unittest.TestCase):
    ...
    def test_fit_rf(self):
        """Tests GridSearchCV fit() with RandomForestClassifier."""
    ...
    def test_fit_csvm(self):
        """Tests GridSearchCV fit() with CascadeSVM."""
        x, y = datasets.load_iris(return_X_y=True)
        ...
        expected_keys = {'param_max_iter', 'params', 'split0_test_score',
                         'split1_test_score', 'split2_test_score',
                         'mean_test_score', 'std_test_score',
                         'rank_test_score'}
        self.assertSetEqual(set(searcher.cv_results_.keys()), expected_keys)

        expected_params = [1, 2, 3, 4]
        for params in searcher.cv_results_['params']:
            m = params['max_iter']
            self.assertIn(m, expected_params)
            expected_params.remove(m)
        self.assertEqual(len(expected_params), 0)
    ...
    def test_scoring_callable(self):
        ...
        with self.assertRaisesRegex(ValueError, 'scoring must return a number'):
            searcher.fit(ds)
