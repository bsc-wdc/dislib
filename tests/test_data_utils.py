...
from dislib.data.util import pad, pad_last_blocks_with_zeros, compute_bottom_right_shape
...
class DataUtilsTest(unittest.TestCase):
    ...
    def test_compute_bottom_right_shape(self, shape, block_size, desired_br_shape):
        np.random.seed(8)
    ...
    def test_pad(self, shape, block_size, value):
        ...
        bottom = 0 if shape[0] % block_size[0] == 0 else block_size[0] - shape[0] % block_size[0]
        right = 0 if shape[1] % block_size[1] == 0 else block_size[1] - shape[1] % block_size[1]

        pad(a, ((0, bottom), (0, right)), constant_value=value)

        a_np = a.collect()
        # check if original content is the same
        self.assertTrue(np.allclose(a_np[0:shape[0], 0:shape[1]], a_original.collect()))
        # check if bottom rows are added correctly
        self.assertTrue(np.allclose(a_np[shape[0]:shape[0] + bottom, :], np.full((bottom, shape[1] + right), value)))
        # check if right columns are added correctly
        self.assertTrue(np.allclose(a_np[0:shape[0] + bottom, shape[1]:shape[1] + right],
    ...
    def test_pad_with_zeros(self, shape, block_size):
        ...
        self.assertTrue(np.allclose(a_np[0:shape[0], 0:shape[1]], a_original.collect()))
        # check if bottom rows are added correctly
        self.assertTrue(np.allclose(a_np[shape[0]:shape[0] + bottom, :], np.full((bottom, shape[1] + right), 0)))
        # check if right columns are added correctly
        self.assertTrue(np.allclose(a_np[0:shape[0] + bottom, shape[1]:shape[1] + right],
